...
def index():
    if 'username' in session:
        ...
        if not user_info.get('is_active', True): 
            session.clear()
            return "<h1>🛑 Account Blocked!</h1><a href='/logout'>Go Back</a>"
...
def login():
    ...
    if username in db["customers"] and db["customers"][username]["password"] == password:
        ...
        if not user_data.get('device_id'): 
            db["customers"][username]["device_id"] = client_device; save_db(db)
...
def set_category():
    ...
    db["customers"][username]["bot_active"] = True
    save_db(db)
    
    trigger_instant_ai_production(username, selected_cat)
    return jsonify({"status": "SUCCESS", "message": "🎯 Category Locked Successfully!"})
